...
import { roofFragments } from '../london-island/damage-fragments';
...
function texture(features,box,palette,size=4096){
 ...
 for(const f of features){
  ...
  if(['Polygon','MultiPolygon'].includes(g.type)&&['landcover','park','water','building'].includes(layer)){
   ctx.fillStyle=layer==='water'?palette.water:layer==='building'?palette.building:palette.park;
...
export async function createIsland(container,style,callbacks,signal){
 ...
 controls.enableDamping=false;controls.minDistance=.12;controls.maxDistance=8;controls.minPolarAngle=.12;controls.maxPolarAngle=82*RAD;
 ...
 const mapTexture=texture(detail.features,worldBox,palette,2048);
 const parts=[],surfaces=[],buildingMeshes=[],buildingRecords=[],fragmentMeshes=[],areaMeshes=[],extraMaterials=[];
 ...
 for(const f of features){
  const result=f.properties.assessment,best=result.status==='candidate'?result.candidates[0]:null;
  const record={id:f.id,sourceId:f.properties.buildingId,name:f.properties.sourceName,release:index.release,
  ...
  for(const piece of f.renderPieces){
   ...
   const damage=best?new THREE.MeshBasicMaterial({color:damageCategories[best.category].color,toneMapped:false}):neutral;
 ...
 function apply(next){
  ...
  for(const p of parts){
   ...
   p.top.color.set(next.palette.modelColour);p.sides.color.set(next.palette.modelColour);
  ...
  for(const m of areaMeshes)m.visible=next.surface.damageColours&&next.surface.damageView==='areas';
 ...
 function pointerUp(e){const start=down;down=null;if(!start||start.moved||e.button!==0||start.id!==e.pointerId)return;
  ...
  const building=values?.surface.buildings?raycaster.intersectObjects(buildingMeshes.filter(m=>m.visible))[0]:null;
 ...
 const resize=new ResizeObserver(()=>{const {width,height}=container.getBoundingClientRect();renderer.setSize(width,height);camera.aspect=width/height;camera.updateProjectionMatrix();if(values)render();});resize.observe(container);
 ...
 const siteRing=sample.site.map(p=>placement.coordinates(p));siteRing.push(siteRing[0]);
 const localMeshes=buildingMeshes.filter(mesh=>{
  const f=features.find(f=>f.id===mesh.userData.record.id);
  return polygonClipping.intersection(polys(f.geometry),[[siteRing]]).length>0;
 });
 const historyGroup=new THREE.Group();parts[0].group.add(historyGroup);
 const historyRows=[];
 for(const row of sample.rows){
  const ring=row.ring.map(p=>placement.coordinates(p));ring.push(ring[0]);
  const geometry=shapeGeometry({type:'Polygon',coordinates:[ring]},1);geometry.translate(0,0,.001);
  const material=new THREE.MeshBasicMaterial({color:'#347b7a',transparent:true,opacity:.2,depthWrite:false,side:THREE.DoubleSide});
  const mesh=new THREE.Mesh(geometry,material);mesh.renderOrder=7;
  const edge=new THREE.LineSegments(new THREE.EdgesGeometry(geometry,25),new THREE.LineBasicMaterial({color:'#245e5d',transparent:true,depthTest:false}));edge.renderOrder=8;
  historyGroup.add(mesh,edge);historyRows.push({mesh,edge});
 }
 historyGroup.visible=false;
 const siteLine=new THREE.Line(new THREE.BufferGeometry().setFromPoints(siteRing.map(p=>new THREE.Vector3(...project(p),.0004))),new THREE.LineDashedMaterial({color:'#554d3e',dashSize:.003,gapSize:.002,transparent:true,opacity:.7,depthTest:false}));siteLine.computeLineDistances();siteLine.renderOrder=6;parts[0].group.add(siteLine);
 function setHistory({mode='today',mix=0,height=8,opacity=.25}={}){
  historyGroup.visible=mode!=='today';
  const modernOpacity=mode==='ghosts'?opacity:mode==='compare'?1-mix:1;
  for(const mesh of localMeshes){mesh.visible=modernOpacity>0;mesh.material=mesh.userData.neutral;mesh.material.transparent=modernOpacity<1;mesh.material.opacity=modernOpacity;mesh.material.depthWrite=modernOpacity>=1;mesh.castShadow=modernOpacity>=1;}
  for(const row of historyRows){
   row.mesh.scale.z=row.edge.scale.z=Math.max(.0003,height/1000);
   row.mesh.material.opacity=mode==='compare'?.65*mix:.1;
   row.edge.material.opacity=mode==='compare'?mix:1;
  }
  container.dataset.mode=mode;container.dataset.mix=String(mix);container.dataset.localBuildings=String(new Set(localMeshes.map(m=>m.userData.record.id)).size);
  render();
 }
 function focusSite(){
  const xy=project(placement.coordinates([681,345]));
  controls.target.set(xy[0],parts[0].group.position.y,-xy[1]);controls.cursor.copy(controls.target);
  orient(57,11,Math.max(.34,.28/camera.aspect));render();
 }
 return {apply,focus,focusSite,setHistory,inspect,setDragMode,zoom:factor=>{camera.position.sub(controls.target).multiplyScalar(factor).clampLength(controls.minDistance,controls.maxDistance).add(controls.target);controls.update();},rotate:angle=>{orient(controls.getPolarAngle()/RAD,controls.getAzimuthalAngle()/RAD+angle);callbacks.onCamera({tilt:Math.round(controls.getPolarAngle()/RAD),bearing:Math.round(controls.getAzimuthalAngle()/RAD)});},
