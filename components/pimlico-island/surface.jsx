...
import '@/app/proto/borough-plates/system/tokens.css';
import './surface.css';
...
export default function Surface({config,definition}){
  ...
  return <main className="borough-study surface-study">
    ...
    <div className="surface-workspace">
      <aside className="surface-sidebar" aria-label="Island appearance">
        <fieldset disabled={!stats}><legend>ON THE SURFACE</legend><div className="surface-layer-options">
          {[['paper','Wartime sheet','The original map, aligned to Pimlico'],['damage','Damage colours','Saved area, on ground and roofs'],['modern','Modern map','Buildings, streets and the Thames']].map(([id,label,note])=><button key={id} aria-pressed={settings.layer===id} disabled={id==='paper'&&!stats?.hasPaper||id==='damage'&&!stats?.hasDamage} onClick={()=>change('layer',id)}><span>{label}</span><small>{note}</small></button>)}
        </div></fieldset>
        {settings.layer==='paper'&&<fieldset disabled={!stats} className="surface-paper-controls"><legend>MAP OVERLAY</legend><label>Paper opacity <output>{Math.round(settings.opacity*100)}%</output><input aria-label="Paper opacity" type="range" min="0" max="1" step=".05" value={settings.opacity} onChange={e=>change('opacity',Number(e.target.value))}/></label><label>Paper extent<select aria-label="Paper extent" value={settings.scope} onChange={e=>change('scope',e.target.value)}><option value="sheet">Whole sheet preview</option><option value="selection">Saved selection only</option></select></label><p>{settings.scope==='sheet'?'Exploratory placement beyond the locally checked area. The river remains visible.':'Uses the exact selection and alignment saved in the map editor.'}</p></fieldset>}
        <fieldset className="surface-toggles" disabled={!stats}><legend>THE ISLAND</legend>{[['buildings','3D buildings'],['river','River Thames'],['dirt','Dirt underside']].map(([id,label])=><label key={id}><input type="checkbox" checked={settings[id]} onChange={e=>change(id,e.target.checked)}/>{label}</label>)}<label className="surface-height">Building height <output>{settings.height}×</output><input type="range" aria-label="Building height" min="1" max="4" step=".25" value={settings.height} onChange={e=>change('height',Number(e.target.value))}/></label></fieldset>
      ...
      <section className="surface-stage" aria-label="Pimlico island preview">
        <div className="surface-stage-top"><div className="surface-views" aria-label="Island viewpoints">{[['island','Island'],['top','Overhead'],['detail','Damage close-up']].map(([id,label])=><button key={id} disabled={!stats} aria-pressed={view===id} onClick={()=>chooseView(id)}>{label}</button>)}</div><span className="surface-evidence">Provisional alignment</span></div>
        <div className="surface-canvas" ref={host}/>
        {!stats&&!error&&<div className="surface-message" role="status">Assembling Pimlico…</div>}
        {error&&<div className="surface-message" role="alert"><p>{error}</p><button onClick={()=>setAttempt(n=>n+1)}>Retry island</button></div>}
        <div className="surface-stage-bottom"><div className="surface-map-caption"><strong>{view==='detail'?'Cambridge Street · Alderney Street':'Pimlico & the Thames'}</strong><span>{stats?`${stats.buildings.toLocaleString('en-GB')} modern building components`:'Loading modern geometry'}</span></div><nav className="surface-camera" aria-label="Camera controls"><button disabled={!stats} aria-label="Rotate left" onClick={()=>engine.current?.rotate(-20)}><RotateCcw size={17}/></button><button disabled={!stats} aria-label="Rotate right" onClick={()=>engine.current?.rotate(20)}><RotateCw size={17}/></button><button disabled={!stats} aria-label="Zoom out" onClick={()=>engine.current?.zoom(1.25)}><Minus size={17}/></button><button disabled={!stats} aria-label="Zoom in" onClick={()=>engine.current?.zoom(.8)}><Plus size={17}/></button><button disabled={!stats} aria-pressed={settings.drag==='pan'} onClick={()=>change('drag',settings.drag==='pan'?'orbit':'pan')}>{settings.drag==='pan'?'Pan':'Orbit'}</button></nav></div>
